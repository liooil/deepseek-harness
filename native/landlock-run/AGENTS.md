...
This directory builds `landlock-run`, a Landlock self-restrict-then-exec launcher: a small, auditable confinement binary kept in private per-platform workspace packages, plus the thin JS entry package that resolves it and implements its CLI contract. It belongs to the repository's root pnpm workspace and lockfile. The main repository owns native CI, internal package assembly, and verification; the desktop executable is the only public release artifact.
...
```text
packages/entry/     Private entry package: JavaScript API (resolve/probe/grants) + the C source.
packages/linux-*/   Private per-platform packages: one prebuilt static binary, no JavaScript.
scripts/            Build, matrix derivation, pack gates, and internal verification.
test/               Plain-node behavioral tests (entry API + real-kernel launcher proofs).
docs/               Architecture, packaging, CLI contract, support matrix, naming.
```
...
- Every internal tarball is gated at pack time: platform packages refuse to pack without their declared binaries present, executable, and in the right ELF architecture (`verify-launcher-binary.mjs`), entry packages without built `lib/` (`verify-entry-lib.mjs`), and the packed-install rehearsal byte-pins installed binaries against the workspace builds (`verify-packed-install.mjs`).
- Platform tarballs are packed with `npm pack`, never `pnpm pack`: pnpm's pack path strips the executable bit (observed on 11.7.0), shipping a launcher no consumer can spawn. `pack-workspace.mjs` encodes the split; the rehearsal asserts executability of the installed copy so a regression fails loudly instead of masquerading as a non-enforcing kernel.
- Generated artifacts stay out of git: `packages/*/bin/`, `packages/*/lib/`, `dist/`, `*.tsbuildinfo`. Ignore rules live in the ROOT `.gitignore` only — a package-nested ignore file can silently drop payload from tarballs.
