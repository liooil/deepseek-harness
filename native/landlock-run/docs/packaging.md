...
Unsupported platforms are intentionally absent from `optionalDependencies` — see [support-matrix.md](support-matrix.md). These packages remain private workspace inputs; the desktop executable is the public distribution artifact.
...
- [support-matrix.md](support-matrix.md) explains why unsupported platform packages are not built.

`scripts/github-matrix.mjs` derives the CI matrix from these files. `scripts/build.ts` builds only the current host's targets, into `packages/<name>/bin/`; it is not a matrix generator. When changing the matrix, update package metadata, `prebuilds.json`, the lockfile, and the support docs in the same change.
