...
[Desktop CI](../../../../.github/workflows/desktop-ci.yml) 中的每个 `pnpm/action-setup` 步骤都把目标目录设置在 `runner.temp` 下，并包含 run id、attempt 和 job 名称。因此 Linux、macOS 与 Windows 上的每个 job 都独占自己的 pnpm 安装目录。可选的 [Python SDK 可执行文件构建](../../../../.github/workflows/build-exe-for-python-sdk.yml)继续使用自己的 run/job 级目标目录。最初的目录冲突与组织 runner 拓扑仍记录在[上游 CI 工作流](https://github.com/deepseek-ai/deepseek-harness/blob/master/.github/workflows/ci.yml)中。

[工作流回归测试](../../../../scripts/ci-workflow.spec.ts)会找出 Desktop CI 中的每个 `pnpm/action-setup` 步骤，并拒绝缺少 job 专属目标目录的步骤。这可确保后续新增的桌面 job 也处于同一隔离边界内。
