...
The Landlock launcher source and package family live at `native/landlock-run`, next to the harness consumers and inside the root pnpm workspace. The workspace owns the shared lockfile, native build, and packed-install validation; the [desktop-only release decision](../process/2026-08-21-desktop-binary-only-release.md) keeps those artifacts out of public package publication. Platform binaries are selected by the local workspace validation, and the entry package owns path resolution, probing, CLI flags, the fatal prefix, and the partial-enforcement notice while the harness maps sandbox modes to grants. Versioning the entry point with its binaries keeps probe parsing and launch syntax aligned.
