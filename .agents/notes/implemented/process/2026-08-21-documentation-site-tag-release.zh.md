...
此 fork 的公开软件只在 `desktop-v*` 发布 tag 上前进，公开源码仓库也只推进到每个发布 commit。文档网站却在每次触及 `docs/`、`website/`、投影器或锁文件的 master 推送上部署，既没有审阅人也没有版本校验。尽管仓库是私有的，该站点无需认证即可访问，因此一次合并会在几分钟内把文档发布到公网：其中包含描述尚无任何已发布产物承载的工作的页面，以及从领先于读者所能获取的一切的源码树生成的参考资料。

## Decision

`docs-pages.yml` 只声明 `workflow_dispatch`，并从 `desktop-v*` tag 发布，这与授权桌面可执行文件集合的 tag 家族相同。发布是从发布 tag 出发的显式动作，绝不作为拉取请求检查出现。

build 作业在构建之前要求 `github.ref_type` 为 `tag`，并要求 `github.ref_name` 等于 `desktop-v<根 package.json 版本>`。该检查直接使用被 dispatch 的 ref 和工作树的权威版本，不依赖包发布脚本，也不需要 checkout 完整历史。

`github-pages` 环境携带 `desktop-v*` 部署 tag 策略和必需审阅人。两层应对的是不同的失效：工作流检查拒绝从错误 ref 发起的 dispatch，而当日后某次工作流编辑不再校验时，环境策略仍会拒绝该次部署。
...
`ci-workflow.spec.ts` 钉住这份工作流：`on` 只携带 `workflow_dispatch`，build 作业把被 dispatch 的 tag 与根版本比较，`DOCS_REPOSITORY_REF` 读作 `master`，deploy 作业保留 `github-pages` 环境。
...
文档、桌面二进制与公开源码仓库会在每个发布 tag 上一同前进，没有哪次合并能独自抵达公开读者。发布的代价是每次发布一次 dispatch，而两次发布之间的文档修复要等到下一个 tag 才会公开。从分支发起的 dispatch，或从一个工作树并不携带其版本的 tag 发起的 dispatch，会在构建运行之前于校验步骤失败。
