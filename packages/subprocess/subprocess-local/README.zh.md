...
- **终端进程所有权**：`spawnTerminal` 在 Bun 上分配 `Bun.Terminal`、在 Node 上分配 `node-pty`，桥接 UTF-8 终端文本，并公开一项须等待的终止操作。Linux 和 macOS 会检查当前前台进程组并向其发送信号，还会在终止顶层 shell 前后清理后代进程。每次前台检查都会保留根进程树中的精确身份；Linux 还会在 POSIX 会话 leader 退出后枚举该会话。因此，之前观察到的 macOS 后代以及同会话 Linux 成员在重新设定父进程后仍受围栏保护，pid/start 身份则防止清理跟随 PID 复用。Windows 没有可检查的 POSIX 前台进程组，因此将终端会话生命周期交给 PTY 后端管理。上层 PTY 后端负责提示符就绪、缓冲区与面向模型的操作。
...
- **终端前台检查仅支持 Linux／macOS**：Linux 精确探针覆盖 x64 与 arm64，macOS 则使用 `ps` 快照。Windows 仍支持终端启动、I/O、退出以及由后端管理的清理，但由于 Windows 没有 POSIX 进程组模型，前台检查与信号发送会返回不可用。
...
- **进程内清理要求退出阶段仍能执行 JavaScript**：直接 `process.exit()` 和普通 JavaScript 致命路径可以发出宿主同步 `exit` 事件。未安装 handler 时，`SIGTERM`、`SIGINT` 或 `SIGHUP` 的默认 OS 处置可能绕过该事件；应用只有安装执行正常 dispose 或调用 `process.exit()` 的 handler 才能覆盖这些信号。`SIGKILL`、fatal OOM、`process.abort()`、native crash、断电，以及任何无法运行 JavaScript 的故障，都需要外部 supervisor、容器 init 或等价的 OS 所有者负责。
