...
这是 [`@deepseek-ai/dsh-code-runtime`](../code-runtime/README.zh.md) seam 的 worker 线程实现：`WorkerThreadCodeRuntime` 会在每次运行中使用一个全新的 Node 或 Bun worker，输入 TypeScript，由宿主侧剥离类型，通过消息端口桥接绑定，输出 `{ value, logs, error? }`。**这是隔离措施，而非安全边界**：其信任立场有意与 bash 等价（参见 [PTC Agent Note](../../../.agents/notes/implemented/feature/2026-06-15-ptc.zh.md)），但提供 bash 没有的隔离：独立 isolate、空环境、堆上限与强制终止。
...
- **在执行上下文中，由宿主侧剥离类型**：程序会包裹在异步函数外壳中，由 Node 的 `node:module.stripTypeScriptTypes` 或 Bun 内置 TypeScript transpiler 处理，然后作为 `AsyncFunction` 的函数体执行，因此顶层 `await`／`return` 可用。Node 只接受可擦除语法；Bun 执行其常规 TypeScript 转换。
...
- **两个独立预算，因为对端不可信**：在 Node 上，`computeMs` 通过 `worker.performance.eventLoopUtilization()` 统计累计 worker 忙碌时间。Bun 暴露了该方法但尚未实现，因此 Bun worker 改为发出心跳：等待工具时事件循环仍可响应，而连续同步代码若在 `computeMs` 内无法发出心跳就会被终止。Bun 路径检测的是一次连续卡死，而不是多段短突发的精确累计 CPU 时间。`maxWallMs` 在两种宿主上都为永不 resolve 的等待兜底。两个预算最终都会调用 `worker.terminate()`；堆溢出表现为 `kind: 'worker-exit'`。
...
源代码模式通过 Node 原生类型剥离加载只包含可擦除语法的 `src/worker.ts`。其传递运行时闭包只包含宿主内置模块和相对源模块，因此全新 checkout 绝不需要兄弟工作区包尚未构建的 `lib/` 导出。worker 本地和会话自有的 JSON 边界都会在消息端口两侧展平并重建已验证值，使应用嵌套永远不会进入 structured clone。Node 构建会把兄弟文件 `lib/worker.cjs` 作为文件系统路径传入，因为 pkg 的虚拟文件系统（VFS）Worker hook 要求 CommonJS；同一路径也可在普通 Node 下使用。Bun 桌面构建则单独 bundle worker，并把该文件入口嵌入 standalone 可执行文件，因为 Bun 要求编译时明确列出 worker 入口。对这个已发布入口路径进行测试的仓库级要求由[测试策略](../../../docs/testing.zh.md)规定。
...
- **Node 类型剥离依赖实验性 `stripTypeScriptTypes` API**：如依赖的行为发生变化，amaro 或 sucrase 是已经点名的直接替代品；Bun 使用自身 transpiler，因此对不可擦除 TypeScript 语法的行为可能不同。
- **计算计量因宿主而异**：Node 采样累计忙碌时间；Bun 检测一次连续无响应区间。到期最多可能超过内部 25 ms 轮询间隔。
