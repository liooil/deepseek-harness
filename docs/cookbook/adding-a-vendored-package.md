...
`package.json` invariants: set `private: true`, omit `publishConfig`, rescope the `name` ([mapping](../rescope.md)) while keeping upstream's `version`/`exports`/`type`, point declaration metadata at `lib/types`, include `.d.ts` and `.d.ts.map` declaration outputs in `files`, and list its Cordis dependencies in `peerDependencies` (matching the upstream manifest). Transitive upstream dependencies must themselves be vendored or already present — vendoring one package often means vendoring its dependency tree (for example, `@cordisjs/plugin-http` pulls `@cordisjs/fetch-file`).
